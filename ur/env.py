...
class MultiValleyMountainCarEnv(gym.Env):
    ...
    def render_old(self, mode="human"):
        ...
        if self.viewer is None:
            ...
            flagx = (self.min_goal_position - self.min_position) * scale
            flagy1 = (self._height(self.min_goal_position) - self.min_height) * scale
            flagy2 = flagy1 + 50
            flagpole_left = rendering.Line((flagx, flagy1), (flagx, flagy2))
            self.viewer.add_geom(flagpole_left)
            flag_left = rendering.FilledPolygon(
                [(flagx, flagy2), (flagx, flagy2 - 10), (flagx + 25, flagy2 - 5)]
            )
            flag_left.set_color(0.8, 0.8, 0)
            self.viewer.add_geom(flag_left)

            flagx = (self.max_goal_position - self.min_position) * scale
            flagy1 = (self._height(self.max_goal_position) - self.min_height) * scale
            flagy2 = flagy1 + 50
            flagpole_right = rendering.Line((flagx, flagy1), (flagx, flagy2))
            self.viewer.add_geom(flagpole_right)
            flag_right = rendering.FilledPolygon(
                [(flagx, flagy2), (flagx, flagy2 - 10), (flagx + 25, flagy2 - 5)]
            )
            flag_right.set_color(0.8, 0.8, 0)
            self.viewer.add_geom(flag_right)

        pos = self.state[0]
        self.cartrans.set_translation(
            (pos - self.min_position) * scale, (self._height(pos)  - self.min_height)* scale
        )

        angle = np.arctan(self._tg_alpha(torch.tensor(pos)))
        self.cartrans.set_rotation(angle)
...
class StandUpEnv(gym.Env):
    ...
    def __init__(self, dt = 1, torque = None):
        self.dt = dt
        self.gravity = 0.025
